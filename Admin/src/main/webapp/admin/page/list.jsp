                ...
                <c:forEach items="${thead}" var="th">
                    <c:choose>
                        ...
                        <c:when test="${th.key == 'accountDetail'}">
                            <td class="p-2 rounded-lg border-b border-blue-gray-50">
                                <button onclick="location.href = '${path.replace(".do", "")}/accountDetail.do?pre=${path}&page=${cur}&id=${detail[status.index]}';"
                                        class="w-full h-full hover:cursor-pointer text-blue-gray-300 hover:text-blue-500 hover:bg-gray-200 rounded-xl">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                        ...
                        <c:when test="${th.key == 'boardDetail'}">
                            <td class="p-2 rounded-lg border-b border-blue-gray-50">
                                <button onclick="location.href = '${path.replace(".do", "")}/boardDetail.do?pre=${path}&page=${cur}&id=${detail[status.index]}';"
                                        class="w-full h-full hover:cursor-pointer text-blue-gray-300 hover:text-blue-500 hover:bg-gray-200 rounded-xl">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                         fill="currentColor" class="size-5">
                                        <path fill-rule="evenodd"
                        ...
                        <c:when test="${th.key == 'recruitDetail'}">
                            <td class="p-2 rounded-lg border-b border-blue-gray-50">
                                <button onclick="location.href = '${path.replace(".do", "")}/recruitDetail.do?pre=${path}&page=${cur}&id=${detail[status.index]}';"
                                        class="w-full h-full hover:cursor-pointer text-blue-gray-300 hover:text-blue-500 hover:bg-gray-200 rounded-xl">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                         fill="currentColor" class="size-5">
                                        <path fill-rule="evenodd"
                        ...
                        <c:when test="${th.key == 'serviceDetail'}">
                            <td class="p-2 rounded-lg border-b border-blue-gray-50">
                                <button onclick="location.href = '${path.replace(".do", "")}/serviceDetail.do?pre=${path}&page=${cur}&id=${detail[status.index]}';"
                                        class="w-full h-full hover:cursor-pointer text-blue-gray-300 hover:text-blue-500 hover:bg-gray-200 rounded-xl">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                         fill="currentColor" class="size-5">
                                        <path fill-rule="evenodd"
...
<script>
  const allowAction = (id) => {
    if (confirm("정말 승인하시겠습니까?")) {
      alert("승인되었습니다!")
      fetch('${path.replace(".do", "")}/allow.do?pre=${path}&page=${cur}&id=' + id, {
        method: 'GET',
      })
      setInterval(() => {
      ...
      location.href = '/admin/${path}?page=${cur}'
    }
  }
  const deleteAction = (id) => {
    if (confirm("정말 삭제하시겠습니까?")) {
      alert("삭제되었습니다!")
      fetch('${path.replace(".do", "")}/delete.do?pre=${path}&page=${cur}&id=' + id, {
        method: 'GET',
      })
      setInterval(() => {
        location.href = '/admin/${path}'
